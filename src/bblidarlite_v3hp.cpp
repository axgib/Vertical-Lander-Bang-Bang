/*------------------------------------------------------------------------------
  ...
  bblidarlite_v3HP Beaglebone Library
  bblidarlitelite_v3HP.cpp

  Modified from Arduino library provided by Garmin
  This library provides quick access to the basic functions of LIDAR-Lite
  via the Beaglebone interface. Additionally, it can provide a user of any
...
void LIDARLite_v3HP::waitForBusy(uint8_t lidarliteAddress)
{
    ...
    // bailout prints error
    if (busyCounter > 9999)
    {
        perror("> bailing out of waitForBusy()");
    }
                           ...
                           uint16_t numBytes, uint8_t lidarliteAddress)
{
    int nackCatcher;
    int file;
    char *filename = "/dev/i2c-1";//get correct i2c bus
    ...
    if (ioctl(file, I2C_SLAVE, lidarliteAddress) < 0) {
    ...
    if (write(file, &regAddr, sizeof(regAddr)) != sizeof(regAddr)) {
    ...
    // Write data bytes
    if (write(file, dataBytes, numBytes) != numBytes) {
        perror("Failed to write data bytes.");
        close(file);
        return;
    }
